	...
	$.fn.lb_box = function(customOptions)
	{
		...
		function LoadData($interior)
		{
			...
			if(options.showClose == true || options.type == "iframe")
			{
				...
				{
					closeSelector = "class='" + options.closeSelector.substr(1) + "'";
				}
				else
				{
					closeSelector = "id='" + options.closeSelector.substr(1) + "'";
				}
				
				closeButton = "<div style='float:right; padding-top:8px; width: 50px;'><a href='javascript:void(0)' " + closeSelector + "><img src='/resources/images/admin/dialog/fancy_close.png' border='0' alt='close'/></a></div>";
			}
			...
			if(options.blank == false)
			{
				if(options.title != "")
				{
					titleHtml = "<div class='title'>" + closeButton + options.title + "</div>";
				}
				innerHtml = "<div class='alertpad'>" + $html.html() + "</div>";
			}
			else
			{
				innerHtml = $html.html();
			...
			if(options.noPad == true)
			{
				...
				$interior.css("width", $(".alertpad", $interior).width()-30);
		...
		function GetHtml($box) 
		{
			...
			if(options.type == "iframe")
			{
				...
				{
					iframeHtml = "<iframe src='" + href + "' frameborder='0' height='" + (options.height-70) + "' width='" + (options.width-30) + "'></iframe>";
				}
				return $("<div></div>").html(iframeHtml);
			}
			else
			{
				if(options.message != "")
				{
					return $("<div></div>").html(options.message);
				}
				...
				else
				{
					...
					{
						return $("<div></div>").load(options.dataSource);
					}
				}
			}
		}
