...
(function($) {
	...
	function drawCalControl (target, opt) {
		$(target).append(
			'<div class="jCal">' + 
					...
					'<div class="month">' + 
						'<span class="monthName">' + opt.ml[opt.day.getMonth()] + '</span> <span class="monthYear">' + opt.day.getFullYear() + '</span>' +
					'</div>' +
					( (opt.ind == ( opt.showMonths - 1 )) ? '<div class="right" />' : '' ) +
			'</div>');
		;
		
		$(document).find('.calWrapper .previous').bind('click', $.extend( {}, opt ),
			function (e) {
		...
		$(document).find('.calWrapper .next').bind('click', $.extend( {}, opt ),
			function (e) {
				if ($('.jCalMask', e.data._target).length > 0) return false;
				$(e.data._target).stop();
				var mD = { w:0, h:0 };
	...
	function drawCal (target, opt) {
		...
		$(target).find('div[id^=' + opt.cID + 'd]:last').after( '<br style="clear:both;" />' );
